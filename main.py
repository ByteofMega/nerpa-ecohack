...
from utils import GeoUtils, MockDetections, ImageUtils, HeatmapGenerator
from drone import DroneController


class MirrikoApp:
    """
    Демо-программа для хакатона:
    1) Загрузить фото мусорного пятна.
    2) Загрузить видео с дрона.
    3) Запустить миссию — оба YOLO имитируются MockDetections.
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Миррико — тепловая карта мусорного пятна (демо без моделей)")
        ...
        # Tk-изображения (нельзя собирать GC)
        self._tk_refs = {}
    ...
    def build_ui(self):
        ...
        # Левая панель — видео с дрона
        left = tk.LabelFrame(
            center, text="Видео с дрона (трансляция)",
            bg=C["panel"], fg=C["accent"], font=("Arial", 12, "bold"),
        )
        ...
        self.lbl_video_info = tk.Label(
            self.video_label, text="Видео не загружено",
            bg="black", fg="lime", font=("Courier", 10),
        )
        self.lbl_video_info.place(x=10, y=10)

        # Правая панель — 3 окна
        ...
        def make_img_panel(parent, title, attr):
            frame = tk.LabelFrame(
                parent, text=title,
                bg=C["panel"], fg=C["text"], font=("Arial", 11),
            )
            frame.pack(fill="both", expand=True, pady=4)
            lbl = tk.Label(frame, bg="black", fg="white",
                           text="—", width=42, height=9)
        ...
        def btn(parent, text, cmd, state="normal", bg=None, fg=None):
            b = tk.Button(
                ...
                font=("Arial", 10, "bold"),
                bg=bg or "#334155", fg=fg or "white",
        ...
        self.btn_start       = btn(bottom, "3. Запустить миссию",
                                   self.start_mission, state="disabled",
                                   bg="#10b981", fg="white")
        self.btn_heatmap     = btn(bottom, "4. Построить тепловую карту",
                                   self.make_heatmap, state="disabled")

        self.lbl_verdict = tk.Label(
            bottom, text="РЕЗУЛЬТАТ: ожидание",
            bg=C["panel"], fg="gray", font=("Arial", 15, "bold"),
        )
    ...
    def load_photo(self):
        path = filedialog.askopenfilename(
            title="Выберите фото мусорного пятна",
        ...
        self.lbl_status.config(
            text="Фото загружено. Шаг 2: загрузите видео с дрона.",
    ...
    def _check_ready(self):
        if self.photo_bgr is not None and self.video_path is not None:
            ...
            self.lbl_status.config(
                text="Готово. Шаг 3: запустите миссию.",
    ...
    def _video_loop(self):
        ...
        while self.video_running:
            ...
            cv2.putText(frame, "DRONE CAM  REC", (48, 34),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

            tk_img = ImageUtils.cv2_to_tk(frame)
            try:
    ...
    def start_mission(self):
        ...
        self.lbl_status.config(
            text="Миссия: YOLO_1 анализирует фото, АНПА плывёт к цели…",
            fg=self.COLORS["accent"],
        )

        # --- YOLO_1 (имитация): детекция на спутниковом фото ---
        h, w = self.photo_bgr.shape[:2]
        self.sat_boxes = MockDetections.satellite_boxes(w, h)

        sat_vis = ImageUtils.draw_boxes(self.photo_bgr, self.sat_boxes)
        self._show(self.sat_label, sat_vis, "_tk_sat_vis")

        # --- Запуск движения АНПА ---
    ...
    def on_drone_arrived(self):
        def _cb():
            self.lbl_status.config(
                text="АНПА прибыл. YOLO_2 анализирует последний кадр…",
                fg=self.COLORS["warning"],
            )

            # Берём последний кадр видео (или синтетику, если видео ещё не успело)
            base_frame = (
                self.last_video_frame
                if self.last_video_frame is not None
                else np.zeros((300, 400, 3), dtype=np.uint8)
            )
            base_frame = cv2.resize(base_frame, (400, 300))

            # --- YOLO_2 (имитация): детекция на кадре с дрона ---
            h, w = base_frame.shape[:2]
            self.uw_boxes = MockDetections.underwater_boxes(w, h)

            uw_vis = ImageUtils.draw_boxes(base_frame, self.uw_boxes)
            self._show(self.uw_label, uw_vis, "_tk_uw_vis")

            self.lbl_status.config(
                text="Обе модели отработали. Шаг 4: постройте тепловую карту.",
                fg=self.COLORS["success"],
            )
    ...
    def make_heatmap(self):
        ...
        heat_img = HeatmapGenerator.overlay_heatmap(self.photo_bgr, combined, alpha=0.6)

        self._show(self.heat_label, heat_img, "_tk_heat")
        self.lbl_status.config(
            text="Тепловая карта построена. Красные зоны — высокая концентрация мусора.",
            fg=self.COLORS["success"],
        )
        self.lbl_verdict.config(
            text="РЕЗУЛЬТАТ: мусорное пятно подтверждено ✓",
            fg=self.COLORS["success"],
        )

    # -------------------------------------------------------- Служебные ---
    def _show(self, label, img_bgr, ref_key):
        """Выводит BGR-изображение в tk.Label и сохраняет ссылку от GC."""
        tk_img = ImageUtils.cv2_to_tk(img_bgr)
    ...
    def update_coords(self):
        def _cb():
            self.lbl_coords_drone.config(
                text=f"АНПА: {self.drone.lat:.4f}, {self.drone.lon:.4f}"
            )
            dist = GeoUtils.calculate_distance(
                self.drone.lat, self.drone.lon,
                self.target_lat, self.target_lon,
            )
            self.lbl_dist_depth.config(
                text=f"Дистанция: {int(dist)} м  |  Глубина: {self.drone.depth:.1f} м"
            )
        self.root.after(0, _cb)
    ...
    def update_telemetry(self):
        if self.drone.is_moving:
            ...
            self.lbl_video_info.config(
                text=f"RSSI: {rssi} dBm  |  FPS: 30  |  Связь: поплавок"
            )
